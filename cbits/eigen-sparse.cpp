...
template <class T>
RET sparse_mul_(void* p1,
    const void* p2, int r2, int c2,
    void* p, int r, int c)
{
    typedef SparseMatrix<T> M;
    matrix<T>(p,r,c) = *(M*)p1 * matrix<T>(p2,r2,c2);
    return 0;
}
API(sparse_mul_, (int code, 
    void* p1,
    const void* p2, int r2, int c2,
    void* p, int r, int c), (p1, p2, r2, c2, p, r, c));

template <class T>
RET sparse_mul__(const void* p1, int r1, int c1,
    void* p2,
    void* p, int r, int c)
{
    typedef SparseMatrix<T> M;
    matrix<T>(p,r,c) = matrix<T>(p1,r1,c1) * *(M*)p2;
    return 0;
}
API(sparse_mul__, (int code, 
    const void* p1, int r1, int c1,
    void* p2,
    void* p, int r, int c), (p1, r1, c1, p2, p, r, c));
